...
public class SimulateFork {
    ...
    public static void createProject(String webUrl) {
        ...
        try {
            ...
            SimulateFork.logger.warn("Simulate multi barrier thread error", e);
        }
        simulateTime.setTotalEndTime(CommonService.getDateTime());


        SimulateFork.logger.debug("simulate web url: " + webUrl);
        SimulateFork.logger.debug("total begin time: " + simulateTime.getTotalBeginTime() + " ,end time: " + simulateTime.getTotalEndTime() + " ,use:" + simulateTime.getTotalTimeLength());
        SimulateFork.logger.debug("browser begin time: " + simulateTime.getBrowserBeginTime() + " ,end time: " + simulateTime.getBrowserEndTime() + " ,use:" + simulateTime.getBrowserTimeLength());
        SimulateFork.logger.debug("div size: " + divElements.size() + " ,begin time:" + simulateTime.getDivBeginTime() + " ,end time:" + simulateTime.getDivEndTime() + " ,use:" + simulateTime.getDivTimeLength());
        SimulateFork.logger.debug("span size: " + spanElements.size() + " ,begin time:" + simulateTime.getSpanBeginTime() + " ,end time:" + simulateTime.getSpanEndTime() + " ,use:" + simulateTime.getSpanTimeLength());
        SimulateFork.logger.debug("img size: " + imgElements.size() + " ,begin time:" + simulateTime.getImgBeginTime() + " ,end time:" + simulateTime.getImgEndTime() + " ,use:" + simulateTime.getImgTimeLength());
    ...
    public static WebDriver browserInit(SimulateTime simulateTime, String webUrl) {
        ...
        try {
            ...
            //超时也接着继续进行
            SimulateFork.logger.warn("Simulate wait exceed 10 seconds error", e);
     ...
     * @param element
     */
    public static void extractTextElement(List<Map<String, Object>> list, int spanId, WebElement element) {
        ...
        if (CommonService.checkNotNull(text)) {
            ...
            map.put(String.valueOf(spanId), layer);
            list.add(map);
        }
    }
    ...
    private static void backgroundAndImg(List<WebElement> divElements, int divId, HashMap<String,
        ...
        for (WebElement element :
            ...
            try {
                ...
                SimulateFork.logger.warn("Simulate backgroundAndImg error:", e);
            }
        }
                    ...
                    try {
                        ...
                        for (WebElement element :
                            ...
                            try {
                                ...
                                SimulateFork.logger.warn("Simulate span element with error: " + e.getMessage());
                            }
                        }
                        simulateTime.setSpanEndTime(CommonService.getDateTime());
                    } catch (Exception e) {
                        SimulateFork.logger.warn("Simulate spanThread with error: " + e.getMessage());
                    }
                    barrer.await();

                } catch (Exception e) {
                    simulateTime.setSpanEndTime(CommonService.getDateTime());
                    SimulateFork.logger.warn("Simulate spanThread outer with error: " + e.getMessage());
                    barrer.reset();
                }
            }
        });
            ...
            public void run() {
                try {
                    try {
                        ...
                        SimulateFork.logger.warn("imgThread with error: " + e.getMessage());
                    }
                    barrer.await();

                } catch (Exception e) {
                    simulateTime.setImgEndTime(CommonService.getDateTime());
                    SimulateFork.logger.warn("imgThread outer with error: " + e.getMessage());
                    barrer.reset();
                }
            }
        });
