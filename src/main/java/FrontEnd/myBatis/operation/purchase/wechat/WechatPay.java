package FrontEnd.myBatis.operation.purchase.wechat;
...
public class WechatPay {
    ...
    public static String weChatPay(Orders orders) throws Exception {
        ...
        packageParams.put(Common.PAY_WECHAT_KEY_OUT_TRADE_NO, orders.getOrder_id());
        //随机字符串，长度要求在32位以内。
        ...
        //订单总金额，单位为分
        Float price = Float.parseFloat(orders.getPrice()) * 100;
        packageParams.put(Common.PAY_WECHAT_KEY_TOTAL_FEE, price);
        //JSAPI 公众号支付, NATIVE 扫码支付, APP APP支付
        packageParams.put(Common.PAY_WECHAT_KEY_TRADE_TYPE, TRADE_TYPE);
        //附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
        packageParams.put(Common.PAY_WECHAT_KEY_ATTACH, URLEncoder.encode(JSON.toJSONString(orders), Common.UTF8));
        ...
        //获取支付二维码URL
        Map map = XMLWechatPayUtil.doXMLParse(resXml);
        String urlCode = (String) map.get(Common.PAY_WECHAT_RES_CODE_URL);
    ...
    public static String weChatPayNotify(Object msg) throws Exception {
        ...
        Map<String, String> packageParams = XMLWechatPayUtil.doXMLParse(text);
        //打印查看微信回传的信息
        ...
        String resXml = "";
        //判断签名是否正确
        if (PayCommonUtil.isTenpaySign(Common.UTF8, packageParams,
                GlobalConfig.getProperties(Common.PAY_WECHAT_API_KEY))) {
            //通知微信.异步确认成功.必写.不然会一直通知后台.八次之后就认为交易失败了.
            resXml = PayCommonUtil.packNotifyResXml(true);

            //判断支付状态是否成功支付
            if (Common.PAY_WECHAT_NOTIFY_RESULT_CODE_SUCCESS
                    .equals((String) packageParams.get(Common.PAY_WECHAT_NOTIFY_RESULT_CODE))) {
                ...
                WechatPay.logger.warn("WeChatPay failure, message: " + packageParams.get(Common.PAY_WECHAT_NOTIFY_ERROR_CODE) +
                        " Error depict: " + packageParams.get(Common.PAY_WECHAT_NOTIFY_ERROR_DEPICT));
            }
        } else {
